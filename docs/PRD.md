...
Delivered as a **hosted web app** (live URL), not a CLI tool. A user
submits a repo + task in the browser and watches the agent's reasoning
update live while it works.

```
Browser (user) — submits repo + task, watches live reasoning trace
        │  WebSocket / SSE
        ▼
...
Backend API (FastAPI) — runs on the cloud VM
   ...
   Plan → Act → Observe → Reflect loop
        ...
        │
        ▼
   Ollama (self-hosted on the same VM) — qwen2.5 / llama3.1,
   decides which tool to call next
        │
        ▼
   Docker Sandbox — isolated execution environment, same VM
        │
        ▼
   Result: Passing code + PR, OR failure report after max attempts
   (streamed back to the browser as it happens)
```
...
| LLM reasoning | Self-hosted via Ollama (qwen2.5 / llama3.1) on the deployment VM |
| Backend API | FastAPI, streaming progress over WebSockets/SSE |
| Frontend | React or a simple HTML/JS page with a live reasoning trace |
| Sandboxing | Docker |
| Code search | ripgrep (regex/literal) → Chroma/FAISS embeddings for larger repos |
...
| Hosting | Cloud VM with Docker support (DigitalOcean/Linode/Hetzner/AWS EC2), Nginx reverse proxy, systemd or Docker Compose |

**Cost note:** self-hosting Ollama on a CPU-only VM keeps monthly cost
predictable (~$20-40/month) but means slower responses (~5-15s per
step) than a GPU instance or a paid hosted API would give. This is a
deliberate trade-off for cost control over speed — worth stating
explicitly in the final writeup/demo rather than treating it as a
hidden limitation.
